...
After execution, use [delivery-contract.md](delivery-contract.md) to plan and assemble exactly one customer folder from the transient workspace. Invoke `python <skill-root>/scripts/assemble_delivery.py --plan <plan.json> --output-root <parent>` only after the delivery plan names a fresh minimum whitelist. Promote the plain-language result, selected outputs, rerun essentials, target/source boundary, material assumptions, rights, and unresolved differences required to understand or reuse the work. Promote a supporting data or validation artifact only when the user requested it or it has independent downstream value that cannot be replaced by the README and deterministic rerun.

Do not expose raw search traces, broad environment inventories, runtime/license probe records, QA overlays, internal manifests, debugging logs, iteration traces, sensitivity details, middleware metadata, redundant attempts, or regenerated tables as customer deliverables by default. Summarize a material fact or promote one purpose-built supporting result only when it meets the customer-value rule. Keep multi-target outcomes isolated so one target's failure cannot alter another's result.
