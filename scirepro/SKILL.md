...
4. **Resolve decision-changing unknowns.** Classify an unknown by its effect on the declared scientific observables, not by its mere absence. Resolve or bound claim-defining unknowns; use a traceable, explicit assumption for a plausible nuisance value; fix incidental randomness to a declared reproducible value and treat presentation as secondary unless it affects meaning or readability. After a focused, bounded authoritative pass, if unavailable or unpublished exact original-case input, configuration, parameters, or realization prevents direct recomputation, switch promptly to `mechanism-reproduction` or `alternative-validation` when either still answers the objective; use `original-case-blocked` only when the missing information is essential to the requested result. Follow [source-environment-audit.md](references/source-environment-audit.md) when external artifacts or uncertain execution capability matter.
