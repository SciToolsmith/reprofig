...
3. **Choose the strongest honest route.** Use `direct-recompute`, `mechanism-reproduction`, `alternative-validation`, `image-derived-reconstruction`, or `original-case-blocked`. With paper support, distinguish research-process reproduction or assessment from a result that is only image-derived. With pixels alone, first assess pixel identifiability through [image-derived-reconstruction.md](references/image-derived-reconstruction.md): digitize only recoverable visible data, reconstruct only identifiable subsets, allow appearance work only when that is the user's objective, and for non-identifiable essential data, mapping, or method immediately choose `original-case-blocked`. State what the route tests and cannot support.
